...
{
    [Info("RustAdminPro", "YourName", "1.0.1")]
    ...
    public class RustAdminPro : RustPlugin
    {
        ...
        void Init()
        {
            ...
            // Регистрируем команды из конфига только если они отличаются от атрибутов
            if (!string.Equals(config.Commands.AdminPanelCommand, "admin", StringComparison.OrdinalIgnoreCase))
                cmd.AddChatCommand(config.Commands.AdminPanelCommand, this, nameof(AdminPanelCommand));
            if (!string.Equals(config.Commands.TeleportCommand, "tp", StringComparison.OrdinalIgnoreCase))
                cmd.AddChatCommand(config.Commands.TeleportCommand, this, nameof(TeleportCommand));
            if (!string.Equals(config.Commands.KitCommand, "kit", StringComparison.OrdinalIgnoreCase))
                cmd.AddChatCommand(config.Commands.KitCommand, this, nameof(KitCommand));

            cmd.AddChatCommand("shop", this, nameof(ShopCommand));
            cmd.AddChatCommand("balance", this, nameof(BalanceCommand));
        ...
        void OpenShopPanel(BasePlayer player)
        {
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                container.Add(new CuiLabel
                {
                    Text = { Text = $"{item.Price} {config.Shop.Currency}", FontSize = 10, Align = TextAnchor.MiddleCenter, Color = HexToRustFormat("#f1c40f") },
        ...
        void ChangeAdminSection(ConsoleSystem.Arg arg)
        {
            ...
            string section = arg.Args?.Length > 0 ? arg.Args[0].ToLower() : null;
        ...
        void BuyItem(ConsoleSystem.Arg arg)
        {
            ...
            string shortName = arg.Args?.Length > 0 ? arg.Args[0] : null;
        ...
        void GiveKitFromUI(ConsoleSystem.Arg arg)
        {
            ...
            string kitName = arg.Args?.Length > 0 ? arg.Args[0] : null;
        ...
        void TeleportToLocation(ConsoleSystem.Arg arg)
        {
            ...
            string locationName = arg.Args?.Length > 0 ? arg.Args[0] : null;
        ...
        void ExecutePlayerAction(BasePlayer admin, BasePlayer target, string action)
        {
            switch (action.ToLower())
            {
                ...
                case "бан":
                    BanPlayer(target, $"Забанен администратором {admin.displayName}");
        ...
        {
            var kit = storedData.Kits.FirstOrDefault(x => x.Name.Equals(kitName, StringComparison.OrdinalIgnoreCase));
            if (kit == null)
            {
        ...
        Vector3 GetLocationPosition(string locationName)
        {
            ...
            {
                case "спавн":
                    var spawnPoint = GameObject.FindObjectOfType<SpawnHandler>();
                    return spawnPoint != null ? spawnPoint.transform.position : new Vector3(0, 0, 0);

                case "bandit camp":
                    var bandit = GameObject.Find("assets/bundled/prefabs/autospawn/monument/medium/bandit_town.prefab");
                    return bandit != null ? bandit.transform.position : Vector3.zero;

                case "outpost":
                    var outpost = GameObject.Find("assets/bundled/prefabs/autospawn/monument/medium/compound.prefab");
                    return outpost != null ? outpost.transform.position : Vector3.zero;

                case "launch site":
                    var launch = GameObject.Find("assets/bundled/prefabs/autospawn/monument/large/launch_site_1.prefab");
                    return launch != null ? launch.transform.position : Vector3.zero;

                default:
                    var customLocation = storedData.TeleportLocations.FirstOrDefault(x => x.Name.Equals(locationName, StringComparison.OrdinalIgnoreCase));
                    return customLocation != null ? customLocation.Position : Vector3.zero;
            }
        }
        ...
        void HandleTeleportCommand(BasePlayer player, string[] args)
        {
            ...
            {
                if (TryParseFloatInvariant(args[0], out float x) &&
                    TryParseFloatInvariant(args[1], out float y) &&
                    TryParseFloatInvariant(args[2], out float z))
                {
        ...
        string HexToRustFormat(string hex, float alpha = 1f)
        {
            ...
            if (hex.Length == 6)
            {
                ...
                return string.Format(CultureInfo.InvariantCulture, "{0} {1} {2} {3}", r, g, b, alpha);
            }
        ...
        void MoneyCommand(BasePlayer player, string command, string[] args)
        {
            ...
            if (!int.TryParse(args[2], NumberStyles.Integer, CultureInfo.InvariantCulture, out int amount))
            {
        ...
        void WarnCommand(BasePlayer player, string command, string[] args)
        {
            ...
            {
                BanPlayer(targetPlayer, "Автобан за 3 предупреждения");
        ...
        void ShowPlayerInventory(BasePlayer viewer, BasePlayer target)
        {
            ...
            var items = GetAllInventoryItems(target).Take(30).ToList();
            int itemsPerRow = 6;
            ...
            for (int i = 0; i < items.Count; i++)
            {
                ...
                container.Add(new CuiLabel
                {
                    Text = { Text = $"x{item.amount}", FontSize = 10, Align = TextAnchor.MiddleCenter, Color = HexToRustFormat("#f1c40f") },
        ...
        void RestartCommand(BasePlayer player, string command, string[] args)
        {
            ...
            int delay = 60;
            if (args.Length > 0 && int.TryParse(args[0], NumberStyles.Integer, CultureInfo.InvariantCulture, out int customDelay))
            {
                delay = customDelay;
            }

            PrintToChat($"{config.ChatPrefix} <color=#e74c3c>СЕРВЕР БУДЕТ ПЕРЕЗАГРУЖЕН ЧЕРЕЗ {delay} СЕКУНД!</color>");

            timer.Once(delay, () =>
            {
                RunServerCommand("quit");
            });
        }
        ...
        void BanPlayer(BasePlayer target, string reason)
        {
            if (target == null) return;

            // Используем консольные команды сервера, так как у BasePlayer нет метода Ban
            string escapedReason = reason.Replace('"', '\'');
            RunServerCommand($"banid {target.UserIDString} \"{escapedReason}\"");
            RunServerCommand("server.writecfg");
            target.Kick(reason);
        }

        List<Item> GetAllInventoryItems(BasePlayer target)
        {
            var all = new List<Item>();
            if (target?.inventory == null) return all;

            if (target.inventory.containerMain != null)
                all.AddRange(target.inventory.containerMain.itemList);
            if (target.inventory.containerBelt != null)
                all.AddRange(target.inventory.containerBelt.itemList);
            if (target.inventory.containerWear != null)
                all.AddRange(target.inventory.containerWear.itemList);

            return all;
        }

        bool TryParseFloatInvariant(string input, out float value)
        {
            return float.TryParse(input, NumberStyles.Float, CultureInfo.InvariantCulture, out value) ||
                   float.TryParse(input.Replace(',', '.'), NumberStyles.Float, CultureInfo.InvariantCulture, out value) ||
                   float.TryParse(input.Replace('.', ','), NumberStyles.Float, CultureInfo.InvariantCulture, out value);
        }
